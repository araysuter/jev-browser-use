import { runtimeVersion } from './version.mjs';
...
const failureCategories = new Set(['credential_missing','credential_permissions','credential_unavailable','provider_invalid','model_invalid','authentication','quota','provider_error','transport','network_dns','network_denied','schema','context_limit','sensitive_context','origin_changed','origin_unavailable','approval_invalid','observation_error','approval_required','blocked','low_confidence','no_progress','loading_timeout','action_error','budget','step_limit']);
...
export async function writeMetrics(outcome, { directory = defaultLogDirectory(), now = new Date() } = {}) {
  try {
    ...
    const record = {
      ...
      failureCategory: failureCategories.has(outcome.handoff) ? outcome.handoff : null,
